...
test("print route renders required Assessor packet sections", async () => {
  const html = await printText("demo=1&pin=03-00-000-000-0001&venue=assessor&today=2026-05-01");
...
test("print route renders PTAB public-data-limit language", async () => {
  const html = await printText(
    "demo=1&pin=03-00-000-000-0001&venue=ptab&today=2026-06-01&borDecisionDate=2026-05-20",
  );
...
test("print route labels user-supplied subject values", async () => {
  const html = await printText(
    "demo=1&pin=03-00-000-000-0030&venue=bor&today=2025-07-10&actualSqft=1400&actualImprovementAv=30000",
  );
